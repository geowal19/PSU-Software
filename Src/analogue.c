...
{
    double mean = meas_mean((double*)ana_vlts_values, ANA_AVERAGING_LEN);

    return mean * ANA_VLTS_ADC_CONV_FACTOR;
}

double ANA_GetOutputCurrent()
{
    double mean = meas_mean((double*)ana_amps_values, ANA_AVERAGING_LEN);

    return mean * ANA_AMPS_ADC_CONV_FACTOR;
}
...
void ADC_CallBack(uint16_t value)
{
    if(ana_conv_channel == ADC_CHN_VLTS)
    {
        ...
        if(ana_vlts_values_ptr == ANA_AVERAGING_LEN) ana_vlts_values_ptr = 0;
    }
    ...
    else
    {
        ...
        // Deal with pointer overflow
        if(ana_amps_values_ptr == ANA_AVERAGING_LEN) ana_amps_values_ptr = 0;
